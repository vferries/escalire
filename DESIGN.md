...
| `ink-wash-*.png`, `ink-splat-*.png`, `ink-accent-*.png` | Taches d'encre **générées** (`tools/gen-ink.mjs`, spec 2026-07-11) — sprites verticaux 20 frames (768×15360, gris+alpha) : éclosion + état final | remplacent `ink-splat.png` (jugé non convaincant par Vincent) ; éclaboussures et aquarelles décoratives |
...
| Plumes qui tombent | Hero (20 max) + Rencontres (14 max). Trajectoires physiques par plume (divergence volontaire vs maquette, spec 2026-07-10 v3) : modèle de plaque en chute libre d'Andersen–Pesavento–Wang (JFM 2005, régime *flutter*) intégré une fois au chargement (`src/lib/feather-physics.js`, différé en idle) puis cuit en animation WAAPI — glisses latérales alternées, bascule aux virages, trajectoire unique et apériodique par plume. Descente −24vh→124vh en ~20.5–35.5s (∝ taille), balancement normalisé 30–80px, assiette simulée amortie ×0.55–0.75. Chaque plume a une petite chance (4–10%) de se retourner (rotateY 180° cuit dans la trajectoire) à chaque virage de glisse. 3 wrappers imbriqués : trajectoire simulée (translate+rotate+retournements WAAPI) / frémissement 3D (bascule rotateX dominante) / plume teintée à plat (26–54px, rachis quasi horizontal 74°–106°, côté courbé toujours en bas, miroir écran aléatoire — pointe à gauche ou à droite, opacité .55–.95, 30% floutées 1.4px). **Rafale** : vélocité de scroll → offset translateY amorti (×0.9/frame) proportionnel à une profondeur aléatoire par plume |
| Taches d'encre | Éclosion « l'encre boit dans le papier » : le masque sprite avance frame par frame (`steps(20, jump-none)`, ~1.4–2.2s) + flou « encre humide » 2.2px→0 (`inkWet`) qui lisse les pas — au chargement pour les encres du hero (`inkBloomIn`), à l'apparition pour les sections (`data-reveal="ink-bloom"`). Vie lente ensuite : `inkBreathe` (propriété `scale`, 17–24s) ou `inkDrift` conservé pour les aquarelles du hero. Coupé en `discret` (frame finale directe) |
| Divers | Cue « défiler » bobbing 2.8s ; hovers boutons : fond + translateY(−2px) |
