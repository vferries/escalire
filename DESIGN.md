...
| Plumes qui tombent | Hero (20 max) + Rencontres (14 max). Plumes à plat en balancement pendulaire (divergence volontaire vs maquette, spec 2026-07-10). 4 wrappers imbriqués : chute ~20.5–35.5s (translateY −24vh→124vh + dérive horizontale apériodique ±10–70px) / balancement pendulaire 3.6–7.5s (arc : point bas à mi-balancement, inclinaison max ±10–24° aux extrêmes — comme une balançoire ; période ∝ amplitude) / frémissement 3D (bascule rotateX dominante, ou vrille rotateY 360° 5–11s pour ~12% des plumes) / plume teintée à plat (26–54px, rachis quasi horizontal 74°–106°, flip aléatoire, opacité .55–.95, 30% floutées 1.4px). Amplitudes par plume via variables CSS sur le wrapper externe. **Rafale** : vélocité de scroll → offset translateY amorti (×0.9/frame) proportionnel à une profondeur aléatoire par plume |
